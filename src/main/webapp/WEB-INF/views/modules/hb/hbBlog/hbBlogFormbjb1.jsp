...
<body>
	...
	<br/>
	<form:form id="inputForm" modelAttribute="hbBlog" action="${ctx}/hb/hbBlog/apply" method="post" class="form-horizontal">
  ...
  <div class="modal-footer">
    ...
    <button  id="tijiao" class="btn btn-primary">提交</button>
		...
		<div class="control-group">
			...
			<div class="controls">
				<c:forEach items="${pfList }" var="pic">
					<span id='img${pic.id }'><a href='${ctxBase}/userfiles/${pic.pfpath }' target="_blank"><image src='${ctxBase}/userfiles/${pic.pfpath }' style='width:50px;height:50px;'/></a></span>
				</c:forEach>
			</div>
		</div>
		<div class="control-group">
			<div class="container" style="border: 1px solid red; height: 500px; overflow-y:auto;">
				${hbBlog.articleData.content }
			</div>
		</div>
		
		
		<div class="form-actions">
			<input id="btnApply" class="btn btn-primary"  type="button"  value="通 过"/>&nbsp;
			<input id="btnNo" class="btn btn-primary"  type="button"  value="驳回修改"/>&nbsp;
			<input id="btnEnd" class="btn btn-primary"  type="button" value="不通过"/>&nbsp;
