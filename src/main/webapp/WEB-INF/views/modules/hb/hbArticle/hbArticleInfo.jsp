...
<div id="myModal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  ...
  <div class="modal-footer">
    ...
    <button  id="tijiao" class="btn btn-primary">提交</button>
		...
		<input type="hidden" name="act.comment" id="comment">
...
<input id="menu.id" name="menu.id" type="hidden" value="${hbArticle.menu.id==null?param['menu.id']:hbArticle.menu.id}"/>		
       ...
       <div class="control-group">
		...
		class="journalId input-xxlarge" style=""/>
			...
			<input  type="text" value="<c:if test="${not empty hbArticle.journal && not empty hbArticle.journal.id}">${hbArticle.journal.name} ${hbArticle.journal.journalYear}（${hbArticle.journal.journalNo }）</c:if>
				" disabled="disabled"
		...
		class="input-xxlarge" style=""/>
		 ...
		 <div class="control-group">
			<label class="control-label"><spring:message code="hb_article.label" text="标签"/>：</label>
                <div class="controls">
				<form:input path="label" htmlEscape="false" maxlength="64" class="input-xxlarge "/>
                </div>
            </div>
		<div class="control-group">
