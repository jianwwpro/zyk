...
<head>
	...
	<script type="text/javascript">
		...
		function integralDetails(id,name){
			/* <spring:message code='sys.update' text='查看详细兑换信息'/> */
			top.$.jBox("iframe:${ctx}/hb/hbRewardsApplyinfo/rewardsHis?userId="+id+"&userName="+name, {title: "", width: 800, height: 600, buttons:{'${langType eq "zh"?"关闭":"close"}': true}});
		}
		
		function expExcel(){
			var action=$("#searchForm").attr("action");
			// ${ctx }/hb/hbUserInfo/expUserRewards
			$("#searchForm").attr("action","${ctx }/hb/hbUserInfo/expUserRewards");
			$("#searchForm").submit();
		...
		function seeRewardsInfoByUser(userId){
			top.$.jBox("iframe:${ctx}/hb/hbRewardsInfo/searchRewards?userId="+userId, {title: "", width: 800, height: 600, buttons:{'${langType eq "zh"?"关闭":"close"}': true}});
		}
	</script>
</head>
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/hb/hbUserInfo/listGroupByRewards"><spring:message code="hb_user_info.table.title"/><spring:message code="sys.list"/></a></li>
...
<%-- 		<shiro:hasPermission name="hb:hbUserInfo:edit"><li><a href="${ctx}/hb/hbRewards/listInfo"><spring:message code="hb_rewards.table.title"/></a></li></shiro:hasPermission>
 --%>	</ul>
	<form:form id="searchForm" modelAttribute="hbUserInfo" action="${ctx}/hb/hbUserInfo/listGroupByRewards" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li class="btns"><a id="btnExp" onclick="expExcel();"  href="#" class="btn btn-primary" ><spring:message code='hb_user_info.export_list'/></a></li>
			...
			<!-- 兑换次数 -->
			<li><label><spring:message code="hb_user_info.applyinfo_count" text="hb_user_info.applyinfo_count"/>：</label>
				<form:input path="applyinfoCount" htmlEscape="false" maxlength="64" class="input-medium"/>
			</li>
		</ul>
		<input type="hidden" id="isRewardsC" name="isRewardsC" value=""/>
		<input type="hidden" id="isApplyinfoCount" name="isApplyinfoCount" value=""/>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				<th>
						<spring:message code="hb_user_info.applyinfo_count" text="hb_user_info.applyinfo_count"/>
						&nbsp;&nbsp;
						<sup ><a href="#" onclick="clkTop('isApplyinfoCount');">＾</a></sup>
						<sub style="margin-left:-12px;" ><a href="#" onclick="clkBottom('isApplyinfoCount')">ｖ</a></sub>
						</th>
		...
		<c:forEach items="${page.list}" var="hbUserInfo">
			<tr>
				...
				<td>
					<a href="#" onClick="seeRewardsInfoByUser('${hbUserInfo.id}');">${hbUserInfo.rewardsC}</a>
				</td>
				<td>
					${hbUserInfo.applyinfoCount}
				</td>
				<%-- <td>
					${hbUserInfo.loginTime}
				</td> --%>
				...
				<%--  ${ctx}/hb/hbRewardsApplyinfo/rewardsHis?userId=${hbUserInfo.id}&userName=${hbUserInfo.realName} --%>
    				<a href="#" onclick="integralDetails('${hbUserInfo.id}','${hbUserInfo.realName}')"><spring:message code="sys.view" text="sys.view"/></a>
